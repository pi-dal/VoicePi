...
`Sources/VoicePi/` remains the single executable target, but production Swift code should now be organized by responsibility instead of being kept flat at the target root. Use `Sources/VoicePi/App/` for app composition, startup, lifecycle coordination, and cross-layer orchestration. Use `Sources/VoicePi/Core/` for domain models, prompt composition, policies, workflow logic, and other logic that should stay as platform-agnostic and dependency-light as practical. Use `Sources/VoicePi/Adapters/` for integrations with system APIs, persistence, networking, update delivery, external ASR services, and other boundary-facing infrastructure. Use `Sources/VoicePi/UI/` for status bar, settings, panels, presentation models, and other AppKit or SwiftUI-facing code. Use `Sources/VoicePi/Support/` only for small cross-cutting helpers that do not fit the other layers cleanly.

Keep bundle resources at the `Sources/VoicePi/` root unless packaging behavior is intentionally being changed. In particular, `Sources/VoicePi/Info.plist`, `Sources/VoicePi/AppIcon.appiconset`, and `Sources/VoicePi/PromptLibrary/` should remain rooted there so app packaging and resource lookup stay stable. Keep new production code in `Sources/VoicePi/` unless it is clearly test-only. `Tests/VoicePiTests/` holds Swift unit tests, and `Tests/*.sh` covers repository scripts and release helpers. `Scripts/` contains the supported developer and packaging entrypoints. Release metadata lives in `Casks/voicepi.rb`; release changelog files live in `docs/changelogs/`; and longer design notes belong in `docs/plans/`.
...
Follow the existing Swift style: four-space indentation, `UpperCamelCase` for types, `lowerCamelCase` for properties and methods, and small focused extensions or helper structs where they improve readability. Match the current file naming pattern such as `AppModel.swift` and `RemoteASRClient.swift`. No Swift source file should exceed 800 lines; split files before they reach that ceiling, and prefer further extraction once a file starts drifting into the 600-700 line range. Shell scripts use POSIX `sh`, start with `set -eu`, and should stay portable and explicit. No formatter or linter is configured, so keep diffs tidy and consistent with nearby code.
